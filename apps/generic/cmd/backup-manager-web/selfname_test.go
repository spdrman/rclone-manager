...
//
// The CLI's rename (0.3.3, core/cliecho/cliname.go) moved `backup-manager`
// to `rbm` and moved the web host to `rbm-web` everywhere AROUND this
// binary: the image ships /rbm-web with the old name symlinked beside it,
// container/compose.yaml and all eleven adapter compose files run /rbm-web,
// distribution/packaging's canonical manifest names it, and the CLI's own
// published exit-code table sends an operator to `rbm-web serve` for the
// same status. What none of that reached was the source of the binary
// itself, which went on printing `backup-manager-web:` in front of every
// diagnostic and `usage: backup-manager-web` at the top of its help. So a
...
// followed the CLI's own sentence to `rbm-web` met a program that
...
//
// Three shapes in this tree spell backup-manager and are NOT a program
...
// (/etc/backup-manager/config), the project, image and compose service
// (ghcr.io/spdrman/backup-manager), and the User-Agent core sends. A check
...
// literal may contain backup-manager immediately after a `/`, and may not
...
// backup-manager only as a path segment.
...
// function" is not the same claim as "the first line says rbm-web".
