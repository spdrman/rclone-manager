...
# one name, `rclone-manager:dockercli-test`. Whichever built last owned
...
#
# Each worktree gets its own marker file under core/cmd/rbm,
...
for i in $(seq 1 "$runs"); do
  ...
  # suite asserts on.
  cat > "$dir/core/cmd/rbm/zz_concurrency_marker.go" <<EOF
package main
...
for ref in "${refs[@]:-}"; do
  if [ "$ref" = "rclone-manager:dockercli-test" ]; then
