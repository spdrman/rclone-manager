...
Both run the identical `compose/rclone-manager.yml`. The only difference
...
```bash
pct set <ctid> --mp0 /srv/rclone-manager,mp=/mnt/rclone-manager
```
...
| --- | --- | --- |
| `/mnt/rclone-manager/state` | SQLite catalog, administrator record | Private application state (§19.2) |
| `/mnt/rclone-manager/backups` | Retained artifacts | The user backup root, a separate security domain |
| `/mnt/rclone-manager/config` | `config.yaml`, mounted writable | Validated before the listener opens; the engine also creates `ssh_keys/` and `known_hosts.d/` here |
| `/mnt/rclone-manager/secrets` | SSH key, pinned `known_hosts`, read-only | Never inside the backup root, never in this repository |
