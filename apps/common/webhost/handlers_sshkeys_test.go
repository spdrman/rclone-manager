...
// TestTestConnection_ReportsTheFourStages is the response growing without
// #211's callers noticing.
func TestTestConnection_ReportsTheFourStages(t *testing.T) {
	tr := newBackupSetsTestRouter(t)
	...
	if len(body.Stages) != 4 {
		t.Fatalf("stages = %d, want 4: connect, host key, authenticate, list. Reporting fewer is the boolean this change exists to replace", len(body.Stages))
	}
	want := []string{"connect", "host_key", "authenticate", "list"}
	for i, stage := range body.Stages {
		if stage.Name != want[i] {
			t.Errorf("stage %d is %q, want %q; the order is the order they are proven in", i, stage.Name, want[i])
		}
		if stage.Status == "" {
			t.Errorf("stage %q has no status", stage.Name)
		}
	}
}
