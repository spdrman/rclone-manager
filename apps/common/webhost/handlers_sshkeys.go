...
func (h *handlers) listSSHKeys(w http.ResponseWriter, r *http.Request) {
	...
	if setup == nil {
		...
		writeError(w, http.StatusInternalServerError, "INTERNAL", "failed to list SSH keys")
		return
	}
...
func (h *handlers) listSSHKeyCandidates(w http.ResponseWriter, r *http.Request) {
	setup := h.setup()
	if setup == nil {
		writeError(w, http.StatusInternalServerError, "INTERNAL", "failed to scan for SSH keys")
		return
	}
