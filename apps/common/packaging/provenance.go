...
func GoLinkedModules(target GoBuildTarget, goos, goarch string) ([]GoModuleRef, error) {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		// A module with no version is either the main module or a
		// local replace, and both are first-party here.
		if version == "" || strings.HasPrefix(path, firstPartyModulePrefix) {
			continue
		}
