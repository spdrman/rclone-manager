...
func cleanFixture(t *testing.T) string {
	...
	mustWrite(t, filepath.Join(root, "compose", "rclone-manager.yml"), `services:
  engine:
...
func TestScanLifecycleCatchesViolations(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mutate: func(t *testing.T, root string) {
				p := filepath.Join(root, "compose", "rclone-manager.yml")
...
`)
		...
		},
		{
			...
			mutate: func(t *testing.T, root string) {
				mustWrite(t, filepath.Join(root, "catalog", "app.yaml"), `name: rclone-manager
...
`)
		...
		},
		{
			...
			mutate: func(t *testing.T, root string) {
				mustWrite(t, filepath.Join(root, "template", "rclone-manager.xml"),
					`<?xml version="1.0"?>`+"\n"+`<Container version="2">
  <Name>rclone-manager</Name>
...
func TestScanSecretsCatchesBundledCredentials(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			file:   "compose/rclone-manager.env",
...
func TestContains(t *testing.T) {
	tests := []struct {
		...
		{"/mnt/user/backups", "/mnt/user/appdata/rclone-manager/state", false},
...
func TestScanForBespokeAuthCatchesAnOwnAuthMechanism(t *testing.T) {
	tests := []struct {
		...
		{"an OIDC block in a compose file", "compose/rclone-manager.yml",
		...
		{"an --auth-mode override", "compose/rclone-manager.yml",
...
func TestScanForOMVPluginCatchesPluginMaterial(t *testing.T) {
	tests := []struct {
		...
		{"a salt state tree", "salt/deploy.yml", "rclone-manager: {}\n", true},
		...
		{"an omv-mkconf hook referenced from metadata", "compose/rclone-manager.yml",
...
func TestCheckForwardedHeaderTrustFailsInBothDirections(t *testing.T) {
	engineTrusting := Service{Name: "rclone-manager", Source: "compose.yml", Environment: map[string]string{"TRUST_FORWARDED_HEADERS": "true"}}
	engineSilent := Service{Name: "rclone-manager", Source: "template.xml", Environment: map[string]string{}}
	uiTrusting := Service{Name: "web-ui", Source: "compose.yml", Environment: map[string]string{"TRUST_FORWARDED_HEADERS": "true"}}
	uiSilent := Service{Name: "web-ui", Source: "compose.yml", Environment: map[string]string{}}
...
# Example procedure
	...
	sha256sum /mnt/user/backups/rclone-manager/canary.bin | tee /root/evidence/canary.sha256
	find /mnt/user/backups/rclone-manager -type f | sort > /root/evidence/before
...
func TestCheckAcceptanceProcedureAcceptsASafeVerifiableProcedure(t *testing.T) {
	if v := CheckAcceptanceProcedure(cleanProcedure, "/mnt/user/backups/rclone-manager", nil); len(v) > 0 {
...
func TestCheckAcceptanceProcedureCatchesDestructiveAndUnverifiableSteps(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mutate:   func(s string) string { return s + "\n\tchown -R 99:100 /mnt/user/backups/rclone-manager\n" },
			wantRule: RuleRecursiveChown,
		},
		{
		...
		},
		{
			...
			mutate: func(s string) string {
				return strings.ReplaceAll(s, "/root/evidence/canary.sha256", "/mnt/user/backups/rclone-manager/canary.sha256")
			},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			v := CheckAcceptanceProcedure(tc.mutate(cleanProcedure), "/mnt/user/backups/rclone-manager", tc.subs)
			if !hasRule(v, tc.wantRule) {
...
storage:
  state:
    hostPath: "/mnt/tank/rclone-manager/state"
  backups:
    hostPath: "/mnt/tank/rclone-manager/secrets"
  configDir:
    hostPath: "/mnt/tank/rclone-manager/config/config.yaml"
  sshKey:
    hostPath: "/mnt/tank/rclone-manager/secrets/id_ed25519"
  knownHosts:
    hostPath: "/mnt/tank/rclone-manager/secrets/known_hosts"
network:
  webPort: 9999
runtime:
  puid: 568
