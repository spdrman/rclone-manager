...
func newAdapters() []newAdapter {
	return []newAdapter{
		{id: "portainer", compose: "compose/rclone-manager.yml", env: "compose/rclone-manager.env", acceptance: "docs/acceptance/portainer-stack-deployment.md"},
		{id: "dockge", acceptance: "docs/acceptance/dockge-stack-import.md"},
		{id: "casaos", compose: "compose/rclone-manager.yml", acceptance: "docs/acceptance/casaos-app-store-install.md"},
		{id: "zimaos", compose: "compose/rclone-manager.yml", acceptance: "docs/acceptance/zimaos-app-store-install.md"},
	}
}
...
func mutateForEquivalence(a AdapterRuntime, property string) (AdapterRuntime, string, bool) {
	...
	switch property {
	case PropRoleSet:
		out.Others = append(out.Others, Service{Name: "rclone-manager-sidecar", Command: []string{"/rbm", "daemon"}})
...
func TestThePortainerTemplateIsAStackTemplateAndNotAPlugin(t *testing.T) {
	const (
		root      = "apps/portainer"
		stackfile = "apps/portainer/compose/rclone-manager.yml"
	)
	tpl, err := ReadPortainerTemplates(Path(filepath.Join(root, "templates.json")))
	if err != nil {
		t.Fatalf("read the App Template: %v", err)
	}
	env, err := ReadEnvFile(Path(filepath.Join(root, "compose", "rclone-manager.env")))
	if err != nil {
...
func TestThePortainerTemplateCheckFailsOnEveryWayItCanBeWrong(t *testing.T) {
	const stackfile = "apps/portainer/compose/rclone-manager.yml"
	good := PortainerTemplates{
		...
		Templates: []PortainerTemplate{{
			...
			Env: []PortainerEnv{
				{Name: "STATE_DIR", Label: "state", Default: "/opt/rclone-manager/state"},
			},
		}},
	}
	...
	env := map[string]string{"STATE_DIR": "/opt/rclone-manager/state"}
	...
	cases := []struct {
		...
		{"defaults a variable to something the env file does not",
			...
			RulePortainerTemplate, "compose/rclone-manager.env declares"},
...
func TestCasaOSAndZimaOSStoreMetadataDescribesTheStackBesideIt(t *testing.T) {
	...
	for _, id := range []string{"casaos", "zimaos"} {
		t.Run(id, func(t *testing.T) {
			path := filepath.Join(PlatformDir(id), "compose", "rclone-manager.yml")
			...
			if v := CheckCasaOSMetadata(id+"/compose/rclone-manager.yml", md, rt, c); len(v) > 0 {
...
func TestTheStoreMetadataCheckFailsOnADeliberateMismatch(t *testing.T) {
	c := MustLoad()
	path := filepath.Join(PlatformDir("casaos"), "compose", "rclone-manager.yml")
