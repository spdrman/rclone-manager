...
func ScanLifecycle(root string) ([]Violation, error) {
	...
	err := filepath.WalkDir(root, func(path string, d fs.DirEntry, err error) error {
		...
		// tsc's incremental cache is a build output on the same terms as
		// dist/, generated by `tsc -b` and gitignored, and it lands next
		// to the sources rather than in a directory of its own. A scanned
		// frontend tree that has ever been built holds one, and flagging
		// it as a disallowed file type would turn "I built the app" into a
		// red gate. It is skipped for what it is, not admitted to the
		// allowlist, so a hand-authored .tsbuildinfo would still be caught
		// by nothing pretending it is source.
		if strings.HasSuffix(d.Name(), ".tsbuildinfo") {
			return nil
		}
