...
// ghcr.io/spdrman/rclone-manager, which canonical.json already carries.
...
func TestRegistryDigestComplaints_CoversEveryCombination(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := registryDigestComplaints("ghcr.io/spdrman/rclone-manager", tc.published, tc.indexDigest, tc.arches)
			if tc.want == "" {
				if len(got) != 0 {
	...
	got := registryDigestComplaints("ghcr.io/spdrman/rclone-manager", true, validIndex, []ReleaseArchitecture{
