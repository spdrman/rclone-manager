...
func TestTheLineFormat(t *testing.T) {
	...
	if got, want := gap.Gap, "no "+cliname.Binary+" equivalent yet"; got != want {
	...
	if got, want := patch.Shell(), cliname.Binary+" backup-set patch api-server/var-backups --stale-after 48h"; got != want {
...
func TestCreateSkipsTheFieldsTheRequestDidNotCarry(t *testing.T) {
	...
	if got, want := partial.Shell(), cliname.Binary+" backup-set create api-server/var-backups --host 10.0.0.14"; got != want {
...
func TestRunAllDueSetsPrintsTheGapAndNotBackupManagerRun(t *testing.T) {
	...
	if len(line.Command) != 0 {
		t.Fatalf("run_cycle printed the command %v; `"+cliname.Binary+" run` opens the service in the operator's own process and runs a cycle THERE, so it is a different act against a different process",
			line.Command)
	}
	...
	if !strings.Contains(line.GapDetail, "not in this engine") {
		t.Errorf("the gap does not say why `"+cliname.Binary+" run` is not the answer: %q", line.GapDetail)
	}
