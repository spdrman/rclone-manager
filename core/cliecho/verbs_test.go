...
// in core/cmd/rbm, which is the only package that can see the
...
// Gaps() is what the guard in core/cmd/rbm reads, and a
...
func TestEveryGapSentenceEchoCanPrintIsDeclared(t *testing.T) {
	...
	check := func(a Action) {
		...
		if !declared[line.GapDetail] {
			t.Errorf("%s %s prints a gap sentence Gaps() does not declare:\n  %s\nThe guard in core/cmd/rbm reads that list, so an undeclared sentence is one nothing checks against the verb tables.",
