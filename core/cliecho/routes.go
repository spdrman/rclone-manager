...
const (
	gapRunCycle = "`" + cliname.Binary + " run` starts a cycle in your own shell, not in this engine, so it is a different act against a different process"

	gapRunBackupSet = "`" + cliname.Binary + " fetch --backup-set <source/backup-set>` runs that set's cycle in your own shell, not in this engine, so it is a different act against a different process"
	...
	gapCandidateConnection = "there is no verb that checks a source that is not saved yet: `" + cliname.Binary + " backup-set create` proves the connection before it writes, and `" + cliname.Binary + " backup-set test-connection <source/backup-set>` re-checks one that exists"
)
...
var routes = map[string]entry{
	...
	key("POST", "/ssh-keys/from-candidate"): {
		why: "there is no verb that imports a key this machine already holds; the id is an opaque handle that would sit on a command line perfectly well, and `" + cliname.Binary + " ssh-key import --candidate ID` would be it",
	},
	...
	key("GET", "/ssh-keys"): {
		why:               "there is no verb that lists the key store, which is why `backup-set patch --ssh-key-id ID` currently takes an id nothing will print for you. `" + cliname.Binary + " ssh-key list` would be it",
		namesShippedVerbs: []string{"backup-set"},
	},
	key("GET", "/ssh/key-candidates"): {
		why: "there is no verb that scans this machine for keys it can offer, so the key the installer generated and mounted is reachable from the browser and not from a terminal. `" + cliname.Binary + " ssh-key discover` would be it",
	},
