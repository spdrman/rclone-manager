				...
				BackupSets: []BackupSet{
					{
						...
						Remote: Remote{
							...
							KnownHosts: "/etc/rclone-manager/known_hosts",
						},
...
func TestKeyExactlyOneSourceRequired(t *testing.T) {
	...
	t.Run("key.file alone is accepted and normalized into KeyFile", func(t *testing.T) {
		...
		r := &cfg.Sources[0].BackupSets[0].Remote
		r.KeyFile = ""
		r.Key = Key{File: "/etc/rclone-manager/id_ed25519"}
		if err := cfg.Validate(); err != nil {
			t.Fatalf("Validate: %v", err)
		}
		if r.KeyFile != "/etc/rclone-manager/id_ed25519" {
	...
	t.Run("key.command alone is accepted", func(t *testing.T) {
		...
		r := &cfg.Sources[0].BackupSets[0].Remote
		r.KeyFile = ""
		r.Key = Key{Command: []string{"/usr/local/bin/op", "read", "op://infra/rclone-manager/private-key"}}
		if err := cfg.Validate(); err != nil {
			t.Fatalf("Validate: %v", err)
		}
	})
...
func TestKeyCommandExecutableMustBeAbsolute(t *testing.T) {
	...
	r := &cfg.Sources[0].BackupSets[0].Remote
	r.KeyFile = ""
	r.Key = Key{Command: []string{"op", "read", "op://infra/rclone-manager/private-key"}}
	err := cfg.Validate()
	if err == nil {
...
func TestKeyPassphraseExactlyOneSourceAllowed(t *testing.T) {
	...
	t.Run("command alone is accepted", func(t *testing.T) {
		cfg := validConfig()
		cfg.Sources[0].BackupSets[0].Remote.Key.Passphrase = Passphrase{Command: []string{"/usr/local/bin/op", "read", "op://infra/rclone-manager/private-key-passphrase"}}
		if err := cfg.Validate(); err != nil {
			t.Fatalf("Validate: %v", err)
		}
	})
...
func TestKeyPassphraseCommandExecutableMustBeAbsolute(t *testing.T) {
	cfg := validConfig()
	cfg.Sources[0].BackupSets[0].Remote.Key.Passphrase = Passphrase{Command: []string{"op", "read", "op://infra/rclone-manager/private-key-passphrase"}}
	err := cfg.Validate()
	if err == nil {
...
func TestKeyEncryptionExactlyOneSourceEnforced(t *testing.T) {
	t.Run("file alone is accepted", func(t *testing.T) {
		cfg := validConfig()
		cfg.KeyEncryption = KeyEncryption{File: "/etc/rclone-manager/key.dek"}
		if err := cfg.Validate(); err != nil {
			t.Fatalf("Validate: %v", err)
		}
	})
	...
	t.Run("command alone is accepted", func(t *testing.T) {
		cfg := validConfig()
		cfg.KeyEncryption = KeyEncryption{Command: []string{"/usr/local/bin/op", "read", "op://infra/rclone-manager/dek"}}
		if err := cfg.Validate(); err != nil {
			t.Fatalf("Validate: %v", err)
		}
	})
	...
	for _, tc := range []struct {
		...
		{"file and env", KeyEncryption{File: "/etc/rclone-manager/key.dek", Env: "BACKUP_KEY_DEK"}},
		{"file and command", KeyEncryption{File: "/etc/rclone-manager/key.dek", Command: []string{"/usr/local/bin/op", "read", "x"}}},
...
func TestKeyEncryptionCommandExecutableMustBeAbsolute(t *testing.T) {
	cfg := validConfig()
	cfg.KeyEncryption = KeyEncryption{Command: []string{"op", "read", "op://infra/rclone-manager/dek"}}
	err := cfg.Validate()
	if err == nil {
...
func TestKeyValidateIsIdempotent(t *testing.T) {
	for _, tc := range []struct {
		...
		{"key.file", Key{File: "/etc/rclone-manager/id_ed25519"}},
