...
func TestRenderIncludesProcessInfoAndGeneratedAt(t *testing.T) {
	...
	wantProcess := `rclone_manager_process_info{binary_version="1.2.3",rclone_version="v1.75.0"} 1`
	if !strings.Contains(out, wantProcess) {
		t.Fatalf("Render output missing process info line %q; got:\n%s", wantProcess, out)
	}
	wantGenerated := "rclone_manager_report_generated_timestamp_seconds 1700000000"
...
func TestRenderEveryMetricFamilyHasHelpAndType(t *testing.T) {
	...
	names := []string{
		"rclone_manager_process_info",
		"rclone_manager_report_generated_timestamp_seconds",
		"rclone_manager_backup_set_state",
		"rclone_manager_backup_set_newest_good_backup_age_seconds",
		"rclone_manager_backup_set_stale_threshold_seconds",
		"rclone_manager_backup_set_pending_deletes",
		"rclone_manager_backup_set_failures",
		"rclone_manager_backup_set_quarantined",
		"rclone_manager_backup_set_quarantined_lost",
		"rclone_manager_backup_set_reinstated_remote_retained",
		"rclone_manager_backup_set_current_transfers",
		"rclone_manager_backup_set_free_bytes",
		"rclone_manager_backup_set_last_successful_poll_timestamp_seconds",
		"rclone_manager_backup_set_last_completed_backup_timestamp_seconds",
		"rclone_manager_backup_set_last_retention_run_timestamp_seconds",
	}
...
func TestRenderStateIsOneHot(t *testing.T) {
	...
	want := []string{
		`rclone_manager_backup_set_state{backup_set="prod/postgres-primary",state="healthy"} 0`,
		`rclone_manager_backup_set_state{backup_set="prod/postgres-primary",state="degraded"} 1`,
		`rclone_manager_backup_set_state{backup_set="prod/postgres-primary",state="stale"} 0`,
		`rclone_manager_backup_set_state{backup_set="prod/postgres-primary",state="failing"} 0`,
	}
...
func TestRenderIncludesOptionalFieldsWhenPresent(t *testing.T) {
	...
	wants := []string{
		`rclone_manager_backup_set_newest_good_backup_age_seconds{backup_set="prod/one"} 90`,
		`rclone_manager_backup_set_free_bytes{backup_set="prod/one"} 123456`,
		`rclone_manager_backup_set_last_successful_poll_timestamp_seconds{backup_set="prod/one"} 1700000100`,
		`rclone_manager_backup_set_last_completed_backup_timestamp_seconds{backup_set="prod/one"} 1700000200`,
		`rclone_manager_backup_set_last_retention_run_timestamp_seconds{backup_set="prod/one"} 1700000300`,
	}
	for _, w := range wants {
...
func TestRenderCounters(t *testing.T) {
	...
	wants := []string{
		`rclone_manager_backup_set_pending_deletes{backup_set="prod/one"} 2`,
		`rclone_manager_backup_set_failures{backup_set="prod/one"} 3`,
		`rclone_manager_backup_set_quarantined{backup_set="prod/one"} 4`,
		`rclone_manager_backup_set_quarantined_lost{backup_set="prod/one"} 1`,
		`rclone_manager_backup_set_current_transfers{backup_set="prod/one"} 2`,
		`rclone_manager_backup_set_stale_threshold_seconds{backup_set="prod/one"} 21600`,
	}
	for _, w := range wants {
		if !strings.Contains(out, w) {
			t.Errorf("expected line %q; got:\n%s", w, out)
...
func TestRenderReportsReinstatedRemoteRetainedPerBackupSet(t *testing.T) {
	...
	want := []string{
		`rclone_manager_backup_set_reinstated_remote_retained{backup_set="prod/one"} 3`,
		`rclone_manager_backup_set_reinstated_remote_retained{backup_set="prod/two"} 0`,
	}
	for _, line := range want {
		if !strings.Contains(out, line) {
			t.Errorf("expected line %q in output:\n%s", line, out)
...
func TestRenderAlwaysEmitsReinstatedRemoteRetained(t *testing.T) {
	...
	if !strings.Contains(Render(report), `rclone_manager_backup_set_reinstated_remote_retained{backup_set="prod/one"} 0`) {
