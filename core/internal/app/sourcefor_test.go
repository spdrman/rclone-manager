...
func TestSourceForForwardsEveryKeySource(t *testing.T) {
	for _, tc := range []struct {
		...
		{"file", config.Key{File: "/etc/rclone-manager/id_ed25519"}},
...
func TestSourceForForwardsEveryPassphraseSource(t *testing.T) {
	for _, tc := range []struct {
		...
		{"file", config.Passphrase{File: "/etc/rclone-manager/id_ed25519.passphrase"}},
		...
		t.Run(tc.name, func(t *testing.T) {
			bs := testBackupSet(t, "/var/backups/postgres")
			bs.Remote.Type = "sftp"
			bs.Remote.Key = config.Key{File: "/etc/rclone-manager/id_ed25519", Passphrase: tc.passphrase}
...
func TestSourceForForwardsKeyEncryption(t *testing.T) {
	for _, tc := range []struct {
		...
		{"file", config.KeyEncryption{File: "/etc/rclone-manager/key.dek"}},
		...
		t.Run(tc.name, func(t *testing.T) {
			bs := testBackupSet(t, "/var/backups/postgres")
			bs.Remote.Type = "sftp"
			bs.Remote.Key = config.Key{File: "/etc/rclone-manager/id_ed25519"}
