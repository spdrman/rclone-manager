...
// for review" — an operator running `rbm activity --severity error` sees a
...
// proof: it drives the real recovery this manager offers (`rbm retry` on a
...
func TestActivitySeverityByEdge_TheTwoWaypointsAreNotErrors(t *testing.T) {
	...
	for _, e := range errBody.Events {
		if e.From == "FAILED" && (e.To == "FAILED" || e.To == "QUARANTINED") {
			t.Errorf("`rbm activity --severity error` selected %s -> %s, a waypoint of a recovery that succeeded, "+
...
func TestActivitySeverityByEdge_ValidateOriginQuarantineStaysRed(t *testing.T) {
	...
	if !sawIt {
		t.Errorf("`rbm activity --severity error` no longer selects REMOTE_RETAINED -> QUARANTINED, "+
...
func clientTSPath(t *testing.T) string {
	...
	// This file lives at core/cmd/backup-manager/badge_663_test.go; three
