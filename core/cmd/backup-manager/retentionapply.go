...
func cmdRetentionApply(args []string) int {
	...
	if !*acknowledge {
		return usageError("retention apply: --acknowledge is required. This deletes the local copy of every backup no retention tier keeps, and a deleted restore point is not recoverable from here, so this asks once rather than assuming. `"+cliname.Binary+" retention %s` prints what it would delete", operands[1])
	}
...
func failRetentionApply(err error) int {
	...
	case errors.Is(err, service.ErrRetentionPlanStale):
		fmt.Fprintf(os.Stderr,
			cliname.Binary+": this backup set changed between the preview above and the apply, so nothing was deleted. That is the guard working: what would run is no longer what was printed. Run the command again to decide against what it holds now (%v)\n", err)
		return exitFailure
	case errors.Is(err, service.ErrRetentionApplyBusy):
		fmt.Fprintf(os.Stderr,
			cliname.Binary+": this deployment is running a cycle right now, so nothing was deleted. A cycle writes the journal rows a retention decision is made from, so an apply waits for it rather than deciding against a moving inventory. Try again once it is finished (%v)\n", err)
		return exitFailure
	default:
		return fail(err)
	}
