...
func backupSetPatch(f *backupSetFlags, id string) int {
	...
	if *f.noVerify && namesAConnectionField(f.fs) {
		...
		fmt.Printf("  prove it afterwards with: "+cliname.Binary+" backup-set test-connection %s\n", id)
	}
...
func backupSetRemoveWith(ctx context.Context, svc backupSetRemover, id string, out io.Writer) int {
	...
	if kept < 0 {
		_, _ = fmt.Fprintf(out, "could not count the backups that stay on storage; they are still there, and `"+cliname.Binary+" artifacts` lists them\n")
	} else {
		_, _ = fmt.Fprintf(out, "%d backup(s) stay on storage and stay listed by `"+cliname.Binary+" artifacts`\n", kept)
	}
...
func createIntoExistingConfig(ctx context.Context, configPath, keyFile string, trustHostKey, noVerify bool, req service.CreateBackupSetRequest) int {
	...
	if code := proveSourceConnection(ctx, route, noVerify, connectionTestFor(req), cliname.Binary+" backup-set test-connection "+req.SourceName+"/"+req.Name); code != 0 {
		return code
	}
...
func createFirstConfig(ctx context.Context, configFile, stateDatabase, keyFile string, trustHostKey, noVerify bool, req service.CreateBackupSetRequest) int {
	if req.RunImmediately {
		...
		return usageError("backup-set create: --run cannot be honoured while writing the first configuration, because there is no running service to submit a cycle to yet. Create the set, then `" + cliname.Binary + " run`")
	}
	...
	if code := proveSourceConnection(ctx, firstRun, noVerify, connectionTestFor(req), cliname.Binary+" backup-set test-connection "+req.SourceName+"/"+req.Name); code != 0 {
		return code
	}
...
func printBackupSet(s service.BackupSet) {
	...
	if s.StaleAfter > 0 {
		...
		fmt.Printf("  stale_after: not reported (the engine that answered serves no stale_after; `" + cliname.Binary + " sources` reads it from the configuration)\n")
	}
	...
	if s.ConnectionUnverified {
		fmt.Println("  connection: not verified (nothing has proven this source; `" + cliname.Binary + " backup-set test-connection " + s.ID + "` clears this)")
	}
}
