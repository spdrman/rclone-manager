...
func cmdRetention(args []string) int {
	...
	if len(unconfigured) > 0 {
		...
		for _, u := range unconfigured {
			...
			fmt.Printf("    Create %s again to put them back under a policy: `"+cliname.Binary+" unconfigured` explains the rest.\n", u.Set)
		}
	}
