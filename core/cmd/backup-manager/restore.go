...
func cmdRestore(args []string) int {
	...
	if *medium == "" {
		return usageError("restore: --medium is required; `"+cliname.Binary+" artifacts %s` lists this backup's copies and the medium each one is on", operands[0])
	}
	if !*acknowledge {
...
func printRestoreSubmission(sub service.RestoreSubmission) {
	...
	fmt.Printf("check on it with:    "+cliname.Binary+" status, or GET /api/v1/operations/%s\n", sub.Operation.ID)
}
