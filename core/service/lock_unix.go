...
// it for the process lifetime and a `backupd status` alongside a
...
// than merely sampled), and without a wait a `backupd status`
...
// a running daemon, so an operator can still run `backupd status`
...
// together (an operator's `backupd status` alongside a live `serve`
