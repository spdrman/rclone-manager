...
var sqliteSideFileSuffixes = []string{"-wal", "-shm"}
...
// time but exists now (a WAL/SHM file a failed migration attempt created,
// for example) — so the filesystem afterward matches the pre-migration
// state exactly, not just the main database file's content.
...
// in path's own directory, fsyncing the temp file before the rename, so a
// reader (or a crash) never observes a partially written file at path.
// Mirrors backupsets.go's writeConfigAtomically, generalised to arbitrary
// byte content rather than a marshalled config.Config — restore (above)
// is what needs a second caller of the same pattern; writeConfigAtomically
// itself is left as-is rather than refactored into this, since that is a
// larger change than this issue's own scope.
