# Generic Docker distribution workflow

Docker Compose has no store and no catalog. What this target owes an administrator is a
workflow they can follow, which §73 sets the precedent for itself: Dockge is supported by
Compose compatibility rather than by packaging, so it gets a documented workflow rather
than a submission bundle. So does this one.

The platform's own documentation, cited so this file can be re-derived rather than trusted:
https://docs.docker.com/compose/compose-file/

The packaging metadata this workflow deploys is under `container/`.
