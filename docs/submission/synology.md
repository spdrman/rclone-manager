# Synology Package Center submission checklist

The submission bundle for Synology Package Center, in the form that store asks for it. The shared materials
...
packaging metadata under `apps/synology/spk/`.

That store's own published requirements: https://help.synology.com/developer-guide/

Built by `apps/synology/cmd/spkctl`, which also verifies the finished `.spk` by
re-deriving each embedded binary's SHA-256 out of the archive. This is the one target that
embeds the core binaries rather than consuming the container image, and therefore the one
target with a real hash-parity check of its own.

Both Synology paths stay live and their materials stay separate: this `.spk`, and Container
Manager plus Compose. A reviewer is handed one bundle, not two overlapping ones.
...
Synology Package Center asks for 3. See `docs/submission/screenshots.md` for what to capture and
