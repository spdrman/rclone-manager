...
packaging metadata under ``.

That store's own published requirements: https://www.ugreen.com/pages/nas-developer

**This checklist is ready and its target is not.** 's has produced no `.UPK`,
so there is nothing to submit yet, and is the work package that submits it.

That is the whole point of this file existing now. consumes this bundle and the
...
than failing, so nothing here can hold 's Phase 5 open. The day the `.UPK` exists,
