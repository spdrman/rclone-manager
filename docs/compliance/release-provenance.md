...
`ghcr.io/spdrman/backup-manager:0.1.0` has been pushed. The image index is
`sha256:533e7540`, signed keylessly through the release workflow's own OIDC identity
with the SBOM attested beside it. `distribution/packaging/canonical.json` records that
as `image.published: true`, and the release manifest records the same fact from the
other side as a real `registry_digest` per architecture, read back with
`docker buildx imagetools inspect` rather than taken from the push's own output. The two
are held together by `TestReleaseManifestRegistryDigestTracksTheCanonicalPublishFlag`,
...
both record `0.1.0`, the tag cut for this release rather than the generator's
