...
carrying an `x-casaos` block, so `apps/casaos/compose/rclone-manager.yml` is both
...
```bash
mkdir -p /DATA/AppData/rclone-manager/state /DATA/AppData/rclone-manager/config \
         /DATA/AppData/rclone-manager/secrets /DATA/Backups/rclone-manager
```
...
```bash
ssh-keygen -t ed25519 -N "" -f /DATA/AppData/rclone-manager/secrets/id_ed25519
ssh-keyscan -t ed25519 <sftp-host> > /DATA/AppData/rclone-manager/secrets/known_hosts
```

**Recurse only over what this step created.** `/DATA/Backups/rclone-manager` is the retained
...
```bash
chown -R 1000:1000 /DATA/AppData/rclone-manager/state /DATA/AppData/rclone-manager/config /DATA/AppData/rclone-manager/secrets
chown 1000:1000 /DATA/Backups/rclone-manager
chmod 600 /DATA/AppData/rclone-manager/secrets/id_ed25519
```
...
- [ ] `/DATA/AppData/rclone-manager/config` is writable by the app's uid and gid
...
```bash
$EDITOR /DATA/AppData/rclone-manager/config/config.yaml
chown 1000:1000 /DATA/AppData/rclone-manager/config/config.yaml
chmod 600 /DATA/AppData/rclone-manager/config/config.yaml
```
...
- [ ] Either `config.yaml` is written into `/DATA/AppData/rclone-manager/config` **before** the install
...
1. In CasaOS, **App Store, Custom Install**, choose **Import** and paste
   `apps/casaos/compose/rclone-manager.yml`, or submit the same file to the
...
- [ ] Private state lands under `/DATA/AppData/rclone-manager/state`
- [ ] Retained artifacts land under `/DATA/Backups/rclone-manager`
- [ ] No SSH private key, `known_hosts`, config file or authentication record
      exists anywhere under `/DATA/Backups/rclone-manager`
...
```bash
sha256sum /DATA/AppData/rclone-manager/state/state.db | tee /root/casaos-before-update.sha256
find /DATA/Backups/rclone-manager -type f -printf '%p %s\n' | sort > /root/casaos-before-update.txt
```
...
```bash
dd if=/dev/urandom of=/DATA/Backups/rclone-manager/acceptance-canary.bin bs=1M count=8
sha256sum /DATA/Backups/rclone-manager/acceptance-canary.bin | tee /root/casaos-canary.sha256
...
- [ ] `/DATA/AppData/rclone-manager/state` still holds the catalogue, so a reinstall
...
- [ ] A backup set configured with a root outside `/DATA/Backups/rclone-manager` is refused
