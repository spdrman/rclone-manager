...
is mounted into the guest at `/mnt/rclone-manager`. On ZFS:

```bash
zfs create -o mountpoint=/srv/rclone-manager rpool/rclone-manager
```

or on a plain directory storage:

```bash
mkdir -p /srv/rclone-manager
```
...
```bash
qm create "$VMID" --name rclone-manager --memory 2048 --cores 2 \
...
- [ ] Host directory visible inside the guest at `/mnt/rclone-manager`
...
pct set "$VMID" --mp0 /srv/rclone-manager,mp=/mnt/rclone-manager
pct start "$VMID"
```
...
scp rclone-manager.tar.gz admin@<guest>:/tmp/
ssh admin@<guest> 'gunzip -c /tmp/rclone-manager.tar.gz | docker load'
```

The compose file reads the image reference from a single `IMAGE` variable in
`apps/proxmox/compose/rclone-manager.env`, so this is one line in one file.
...
```bash
ssh admin@<guest> 'sudo mkdir -p /mnt/rclone-manager/{state,backups,config,secrets}'
```
...
```bash
ssh admin@<guest> 'ssh-keygen -t ed25519 -N "" -f /mnt/rclone-manager/secrets/id_ed25519'
ssh admin@<guest> 'ssh-keyscan -t ed25519 <sftp-host> > /mnt/rclone-manager/secrets/known_hosts'
```
...
**Recurse only over what this step created.** `/mnt/rclone-manager` is the shared
host directory, and `/mnt/rclone-manager/backups` is the retained backup store: on
...
```bash
ssh admin@<guest> '
  sudo chown -R 1000:100 /mnt/rclone-manager/state /mnt/rclone-manager/config /mnt/rclone-manager/secrets
  sudo chown 1000:100 /mnt/rclone-manager /mnt/rclone-manager/backups
  sudo chmod 600 /mnt/rclone-manager/secrets/id_ed25519
  sudo -u "#1000" cat /mnt/rclone-manager/secrets/id_ed25519 > /dev/null && echo readable
'
```

- [ ] `/mnt/rclone-manager/{state,backups,config,secrets}` exist, owned by the app's uid/gid
...
- [ ] `/mnt/rclone-manager/config` exists and is **writable** by the app's uid/gid
...
```bash
ssh admin@<guest> 'mountpoint -q /mnt/rclone-manager && echo mounted'
```

- [ ] `mountpoint -q /mnt/rclone-manager` succeeded in the guest, before `up -d`

```bash
scp apps/proxmox/compose/rclone-manager.yml admin@<guest>:/opt/rclone-manager/
scp apps/proxmox/compose/rclone-manager.env admin@<guest>:/opt/rclone-manager/.env
ssh admin@<guest> 'cd /opt/rclone-manager && docker compose -f rclone-manager.yml up -d'
```
...
```bash
qm clone "$VMID" "$((VMID + 1))" --name rclone-manager-repro   # or pct clone
```
...
- [ ] The only edits needed were inside `rclone-manager.env`
...
- [ ] State lands under the host path mapped to `/mnt/rclone-manager/state`
- [ ] Retained artifacts land under the host path mapped to `/mnt/rclone-manager/backups`
...
```bash
ssh admin@<guest> '
  sha256sum /mnt/rclone-manager/state/state.db | tee /tmp/before-update.sha256
  find /mnt/rclone-manager/backups -type f -printf "%p %s\n" | sort > /tmp/before-update.txt
'
ssh admin@<guest> 'cd /opt/rclone-manager && docker compose pull && docker compose up -d'
ssh admin@<guest> '
...
```bash
dd if=/dev/urandom of=/srv/rclone-manager/backups/acceptance-canary.bin bs=1M count=8
sha256sum /srv/rclone-manager/backups/acceptance-canary.bin | tee /root/pve-canary.sha256
find /srv/rclone-manager -type f -printf '%p %s\n' | sort > /root/pve-before-destroy.txt
```
...
- [ ] A backup set configured with a root outside `/mnt/rclone-manager/backups` is refused
...
- [ ] Nothing under `/mnt/rclone-manager/{state,config,secrets}` is ever a delete target
...
| Host storage / dataset used for `/mnt/rclone-manager` | |
