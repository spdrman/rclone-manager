...
`apps/zimaos/compose/rclone-manager.yml` is both the runtime definition and the
store submission.
...
1. In ZimaOS, install the app from its store, or use the custom-install route
   with `apps/zimaos/compose/rclone-manager.yml`.
...
```bash
sha256sum /DATA/AppData/rclone-manager/state/state.db | tee /root/zimaos-before-update.sha256
...
sha256sum /DATA/Backups/rclone-manager/acceptance-canary.bin | tee /root/zimaos-canary.sha256
