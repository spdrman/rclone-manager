...
and `backup-manager-ui` never starts at all: `apps/zimaos/compose/backup-manager.yml` gates it on
...
**The store install path has no shell step.** ZimaOS starts the stack the moment
the tile is clicked, and the install dialog places no files, so `config.yaml` has to
be on the host before that click. Put it there over SSH or through the ZimaOS file
