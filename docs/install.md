...
```
python3 install_docker_host.py install --release 0.3.0
```
...
```
  ok   installing ghcr.io/spdrman/backup-manager:0.3.1
  ok   ghcr.io/spdrman/backup-manager:0.3.1 is sha256:..., the identity the release
       manifest records for 0.3.1
```
...
```
  ok   installing ghcr.io/spdrman/backup-manager:0.3.1
  !!   0.3.1 is cut and not pushed, so container/release-manifest.json records no
       identity for it and there is nothing here to hold
       ghcr.io/spdrman/backup-manager:0.3.1 to.
```

That is 0.3.1 today. It is a warning and never a refusal, and the difference is the
whole design: the alternative was to move the version and leave 0.3.0's digest behind,
which compares a perfectly correct 0.3.1 image against the previous release's identity
