...
There are two halves, and they are not redundant.
  ...
  scripts/bdtools/format/check_gofmt.py  covers every tracked .go file
...
does not use `scripts/bdtools/selftest_swap.py`'s anchored-mutation
...
`scripts/bdtools/selftest/check_anchors.py` runs it.

`python3 scripts/bdtools/format/selftest.py --check-anchors` is that
