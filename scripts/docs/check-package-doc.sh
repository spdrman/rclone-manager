...
# names.
#
# The gate itself is scripts/rcmtools/docs/check_package_doc.py now (EPIC
# I, I1.6 / #672: sixty scripts across twelve domains, each reinventing
# the same four helpers, consolidated onto scripts/rcmtools). This file
...
#   * scripts/rcmtools/docs/selftest.py drives THIS path rather than the
...
# it.
#
# `exec`, so the exit status is the gate's own. See rcmtools.harness for
# where that status is decided.
set -euo pipefail

repo_root="$(cd "$(dirname "$0")/../.." && pwd)"
exec python3 "$repo_root/scripts/rcmtools/docs/check_package_doc.py" "$@"
