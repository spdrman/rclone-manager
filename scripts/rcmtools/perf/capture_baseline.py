...
a `go test` harness's own exit status becomes this script's, unguarded
  hazard in bash:   `set -e` on an unguarded `go test ...` makes the
                    ...
                    shells out to `rbm` -- a capture that never finished
...
USAGE = """\
usage: python3 scripts/rcmtools/perf/capture_baseline.py [options]
...
def body(args: Args, root: Path) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="rclone-manager-perf-") as tmp_str:
        tmp = Path(tmp_str)
        ...
        if not args.skip_image:
            ...
            tag = f"rclone-manager-perfbaseline:{commit[:12]}"
            harness.sh(
