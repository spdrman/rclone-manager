...
reading as a pass. `scripts/rcmtools/conformance/selftest.py` is the same
...
`python3 scripts/rcmtools/compat/selftest.py --check-anchors` is that
...
`core/cmd/backup-manager/usagepins_test.go` and two files under
`core/tests/compat` cite it in their own comments. This file consumes
`scripts/rcmtools/selftest_swap.py`'s `AnchorTracker` rather than
...
a go test failure silently discarded rather than read as "caught"
  ...
  held by:          `compat_gate`/`unit_gate` below always run with
                     ...
                     None of these calls goes through `rcmtools.harness`,
...
the anchor actually landing in the copy the gate then runs
  ...
  hazard in python: GONE as a duplication risk: there is exactly one
                     ...
                     `scripts/rcmtools/selftest_swap.py`, and this module
...
current working directory
  ...
  hazard in python: a DIFFERENT resolution rule, and this exact
                     ...
                     to. `scripts/rcmtools/conformance/selftest.py`,
...
def expect_unit_check_fails(
    ...
    from cmd/backup-manager's own guard test."""
    if tracker.stale_verdict(label):
        return
    if tracker.anchors_only(label):
        return
...
def body(root: Path, dry_run: bool) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="rclone-manager-compat-selftest.") as tmp_name:
        tmp = Path(tmp_name)
        ...
        plant_migration(
            d,
            "-- PLANTED VIOLATION (scripts/rcmtools/compat/selftest.py). A backfill that helps\n"
        ...
        plant_migration(
            d,
            "-- PLANTED VIOLATION (scripts/rcmtools/compat/selftest.py). Discovery time is\n"
        ...
        plant_migration(
            d,
            "-- PLANTED VIOLATION (scripts/rcmtools/compat/selftest.py). A backfill that takes\n"
        ...
        plant_migration(
            d,
            "-- PLANTED VIOLATION (scripts/rcmtools/compat/selftest.py). An upgrade may add to\n"
        ...
        plant_migration(
            d,
            "-- PLANTED VIOLATION (scripts/rcmtools/compat/selftest.py). FR-29 put placements in\n"
        ...
        # breaks this clause.
        tracker.swap(
            d / "core/cmd/backup-manager/artifacts.go",
        ...
        tracker.swap(
            d / "core/cmd/backup-manager/retention.go",
            "\tdefault:\n\t\treturn \"\"\n\t}\n}",
            '\tdefault:\n\t\treturn " medium=local"\n\t}\n}',
        )
        expect_cell_fails(
        ...
        tracker.swap(
            d / "core/cmd/backup-manager/main.go",
        ...
        # new.
        #
        # The guard that closes it lives in core/cmd/backup-manager rather
        ...
        plant_migration(
            d,
            "-- PLANTED VIOLATION (scripts/rcmtools/compat/selftest.py). The same backfill as\n"
