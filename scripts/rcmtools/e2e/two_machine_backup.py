...
`rcmtools.harness.render_help` for why one renderer with one rule beats a
...
from rcmtools.harness import cannot_run, die, note, step, wait_or_die
...
PAYLOAD_PASS = "rclone-manager-e2e-356"
...
class Proof:
    ...
    def __init__(self, repo_root: Path, case_list: list[str], keep: bool) -> None:
        ...
        self.label_key = "rclone-manager-e2e"
        ...
        # scripts/rcmtools/e2e/run_tests_repo_gate.py gives for its own
        ...
        self.product_image = "rclone-manager-e2e:" + self.run_id
        self.source_image = "rclone-manager-e2e-source:1"
        self.machine_image = "rclone-manager-e2e-machine:1"
    ...
    def bm(
        ...
        *args: str,
        ...
        verdict: see rcmtools.harness.EXIT_CANNOT_RUN for what keeps the two
        apart, and scripts/tests/two-machine-exit-status.test.sh for the
        proof that it does.
        """
        return self.mgr_compose(
            mgr, prefix, "exec", "-T", "rclone-manager", "/rbm", *args, check=check, capture=capture
        )
    ...
    def engine_answers(self, mgr: str, prefix: str) -> bool:
        ...
        return harness.sh_ok(self.compose_argv(mgr, prefix, "exec", "-T", "rclone-manager", "/rbm", "version"))
    ...
    def engine_log(self, mgr: str, prefix: str) -> str:
        ...
        """
        out: str = self.mgr_compose(mgr, prefix, "logs", "rclone-manager", check=False).stdout
        return out
    ...
    def build_images(self) -> None:
        ...
        if dirty:
            # Same convention as scripts/rcmtools/e2e/run_tests_repo_gate.py:
    ...
    def run_case(self, case_name: str) -> None:
        ...
        cycled = self.bm(mgr, prefix, "run", "--config", "/etc/backup-manager/config", check=False, capture=False)
        ...
        health = self.bm(
            mgr, prefix, "status", "--config", "/etc/backup-manager/config", check=False, capture=False
        )
    ...
    def _install_for_case(self, case_name: str, mgr: str) -> tuple[str, str]:
        if case_name == "no-arguments":
            ...
            return "/root/rclone-manager", _combined(done)
    ...
    def _assert_installed_version(self, mgr: str, prefix: str) -> None:
        ...
        # fails here. 0.3.3 is a clean cut: the image carries /rbm and
        # nothing else, so there is no second spelling to allow for.
        if "rbm " + self.version not in lines:
            die(
    ...
    def _create_backup_set(self, case_name: str, mgr: str, prefix: str, source_ip: str) -> None:
        ...
        if self.bm(mgr, prefix, "sources", "--config", "/etc/backup-manager/config", check=False).returncode == 0:
            die(
        ...
        self.mgr_compose(mgr, prefix, "stop", "rclone-manager", check=False)
        ...
        self.mgr_compose(mgr, prefix, "start", "rclone-manager")
    ...
    def run_source_verification(self, mgr: str, prefix: str, source_ip: str) -> None:
        ...
        checked = self.bm_stopped(
            ...
            "--config", "/etc/backup-manager/config", check=False,
        )
        ...
        for set_id in ("e2e/nobody", "e2e/offline"):
            if self.bm_stopped(
                mgr, prefix, "backup-set", "remove", set_id, "--config", "/etc/backup-manager/config",
                check=False,
            ).returncode != 0:
    ...
    def run_activity_diagnostic(self, mgr: str, prefix: str) -> None:
        ...
        feed = self.bm_routed(mgr, prefix, "activity", "--config", "/etc/backup-manager/config", check=False)
        feed_err = self.case_dir / "activity.err"
        feed_err.write_text(feed.stderr, encoding="utf-8")
        if feed.returncode != 0:
            die(
                "`rbm activity` failed against a deployment that has just completed a backup.",
                "stderr: " + feed.stderr,
            )
        ...
        as_json = self.bm_routed(
            mgr, prefix, "activity", "--config", "/etc/backup-manager/config", "--limit", "1", "--json",
            check=False,
        )
        ...
        down = self.mgr_compose(
            ...
            "rclone-manager", "/rbm", "activity", "--config", "/etc/backup-manager/config",
            check=False,
        )
    ...
    def _create_admin(self, mgr: str, prefix: str, why: str) -> None:
        ...
        made = self.mgr_compose(
            ...
            "/rbm-web", "auth", "create-admin", "--username", self.admin_user, "--password-stdin",
            check=False,
    ...
    def count_sets(self, mgr: str, prefix: str) -> int:
        ...
        """
        out = self.bm(mgr, prefix, "sources", "--config", "/etc/backup-manager/config", check=False).stdout
    ...
    def count_artifacts(self, mgr: str, prefix: str) -> int:
        ...
        """
        out = self.bm(mgr, prefix, "artifacts", "--config", "/etc/backup-manager/config", check=False).stdout
        counts = [
            int(found.group(1))
    ...
    def run_retention_apply(self, mgr: str, prefix: str) -> None:
        ...
        applied = self.bm_stopped(
            ...
            "--config", "/etc/backup-manager/config", "--acknowledge",
            check=False,
        )
        ...
        health = self.bm(mgr, prefix, "status", "--config", "/etc/backup-manager/config", check=False).stdout
    ...
    def run_empty_record(self, mgr: str, prefix: str, backups: str) -> None:
        ...
        produced by the product's own `rbm reconcile` rather than written by
        hand.
        """
    ...
    def _journal_record(self, mgr: str, prefix: str, name: str) -> dict[str, str]:
        """One artifact's record as the REAL `rbm` reports it.
        ...
        Worth stating plainly, because it is part of the defect: `rbm
        ...
        """
        out = self.bm(
            mgr, prefix, "artifacts", self._artifact_id(name), "--config", "/etc/backup-manager/config"
        ).stdout
        record = {}
    ...
    def _assert_records_match_disk(self, mgr: str, prefix: str, backups: str) -> None:
        ...
        records -- the journal, through the shipped `rbm`, and the sidecar
    ...
    def _walk_the_dead_end(self, mgr: str, prefix: str, backups: str) -> None:
        ...
        walk.append("validate: " + _verdict(self.bm(
            mgr, prefix, "validate", artifact, "--config", "/etc/backup-manager/config", check=False)))

        retried = self.bm(
            mgr, prefix, "quarantine", "retry", artifact, "--config", "/etc/backup-manager/config", check=False
        )
        ...
        for cycle in (1, 2):
            self.bm(mgr, prefix, "run", "--config", "/etc/backup-manager/config", check=False)
            detail = self._journal_record(mgr, prefix, name)
            ...
            if cycle < 2 and detail.get("state") == "FAILED":
                again = self.bm(mgr, prefix, "retry", artifact, "--config", "/etc/backup-manager/config", check=False)
        ...
        walk.append("reconcile (again): " + _verdict(self.bm(
            mgr, prefix, "reconcile", "--config", "/etc/backup-manager/config", check=False)))
    ...
    def _verbs_that_engage(
        ...
        for verb in ("quarantine revalidate", "quarantine reinstate"):
            result = self.bm(
                mgr, prefix, *verb.split(), artifact, "--config", "/etc/backup-manager/config", check=False
            )
...
def _verdict(proc: subprocess.CompletedProcess[str]) -> str:
    ...
    The engine's structured startup lines are dropped. Every `rbm` call logs
    `rbm starting` and `embedded rclone version` as JSON on stderr before it
