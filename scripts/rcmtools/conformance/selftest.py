...
reading as a pass. `scripts/rcmtools/compat/selftest.py` is the same
...
`python3 scripts/rcmtools/conformance/selftest.py --check-anchors` is that
check on its own: every anchor against the real tree, building nothing, in
about a second. `scripts/rcmtools/selftest/check_anchors.py` runs it for
...
a go test failure silently discarded rather than read as "caught"
  ...
  held by:          `conformance_gate`/`unit_gate`/`unit_gate_multi` below
                     ...
                     `rcmtools.harness.sh`, whose whole point is the
...
def body(root: Path, dry_run: bool) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="rclone-manager-conformance-selftest.") as tmp_name:
        tmp = Path(tmp_name)
        ...
        #
        # They live here rather than in scripts/rcmtools/compat/selftest.py
