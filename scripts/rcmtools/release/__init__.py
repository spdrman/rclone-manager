...
`scripts/rcmtools/tests/record_release_hashes_guards.py` and
`scripts/rcmtools/tests/publish_image_guards.py`, through the same
...
same shape as `scripts/rcmtools/api/selftest.py` driving
`scripts/api/check-contract-drift.sh`.

Nothing calls a second Python copy of those suites: the two under
`scripts/rcmtools/tests/` are the only ones, and no assertion from
