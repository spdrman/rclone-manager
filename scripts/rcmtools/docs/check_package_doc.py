...
it says, confirm the new carrier list is the one you meant, and record it:
  ...
  python3 scripts/rcmtools/docs/check_package_doc.py --update
...
def body_against(root: Path, against: str) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="rclone-manager-package-doc-against.") as atmp_name:
        atmp = Path(atmp_name)
        tree_dir = atmp / "tree"
        tree_dir.mkdir()
...
def body(root: Path, update: bool, against: str | None) -> int:
    ...
    details.append("    python3 scripts/rcmtools/docs/check_package_doc.py --update")
