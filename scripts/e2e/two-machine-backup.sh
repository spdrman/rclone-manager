...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      render_help
      exit 0 ;;
    *) die "unknown option $1" "Usage: $0 [--case plain|no-arguments|connection-cap|activity-diagnostic|lifecycle|all] [--keep-on-failure]" ;;
  esac
done

case "$cases" in
  all) case_list="plain no-arguments connection-cap activity-diagnostic lifecycle" ;;
  plain|no-arguments|connection-cap|activity-diagnostic|lifecycle) case_list="$cases" ;;
  *) die "unknown case $cases" "Cases are: plain, no-arguments, connection-cap, activity-diagnostic, lifecycle, all." ;;
esac
