...
| `artifacts` | list journal artifacts, optionally filtered by `--source` and `--backup-set`. `--backup-set` takes the `source/backup-set` id this list prints in its own first column, and a plain set name where one source configures it; a name two sources share is refused with both ids rather than answered for one of them (issue #569) |
