 ...
 *
 * Apply stays unavailable until all four pass, and it is disarmed by any
 * change to what was verified: `verifiedAgainst` records the exact key
 * and host line the four greens were about, so editing either one after a
 * pass turns Apply off rather than letting a green stand for values
 * nobody checked.
 *
...
function OpenWizard({
  ...
  const allPassed =
    outcome !== null &&
    outcome.stages.length > 0 &&
    outcome.stages.every((s) => s.status === "passed") &&
    !staleResults;
...
function VerifyStep({
  ...
  const failed = outcome?.stages.find((s) => s.status === "failed");
  return (
    <>
      <p style={lede}>
      ...
      {outcome === null ? null : outcome.stages.length === 0 ? (
        <Notice tone="warn">
          {"This engine reported " +
            (outcome.ok ? "success" : "a failure") +
            " without a per-check breakdown, so there are no four results to show. " +
            (outcome.message ?? "")}
        </Notice>
      ) : (
        <ol style={checkList}>
          {outcome.stages.map((s) => (
            <li key={s.name} style={checkRow}>
              <span style={{ ...checkGlyph, color: glyphColour(s.status) }}>{glyph(s.status)}</span>
              <span style={{ flex: 1, minWidth: 0 }}>
                <span style={{ fontWeight: 600 }}>{stageTitle(s, user)}</span>
                {s.detail ? (
                  <span style={{ ...mono, display: "block", color: "var(--text-3)" }}>{s.detail}</span>
                ) : null}
              </span>
              <span style={quietInline}>
                {s.status === "skipped" ? "not attempted" : s.durationMs + " ms"}
              </span>
            </li>
          ))}
        </ol>
      ...
      {failed?.name === "authenticate" && publicKey ? (
        <Notice tone="warn">
      ...
      <p style={quiet}>
        Apply stays unavailable until all four pass. A green &ldquo;reached the server&rdquo;
        on its own proves a socket opened; the line that matters to a backup is the last
        one.
      </p>
    </>
  );
...
function stageTitle(s: ConnectionTestStage, user: string): string {
  switch (s.name) {
    case "connect":
    ...
    case "authenticate":
      return s.status === "failed" ? "Could not authenticate as " + user : "Authenticated as " + user;
    case "list":
      return "Listed the folder this set pulls from";
  }
}

function glyph(status: ConnectionTestStage["status"]): string {
  return status === "passed" ? "PASS" : status === "failed" ? "FAIL" : "SKIP";
}

function glyphColour(status: ConnectionTestStage["status"]): string {
  return status === "passed" ? "var(--ok)" : status === "failed" ? "var(--danger)" : "var(--text-3)";
}
