...
export interface ConnectionTestOutcome {
  ...
  message?: string;
  /**
   * Issue #592: the four claims a verification makes, in the order they
   * are proven, including the ones never attempted.
   *
   * `ok` and `message` above are exactly what they always were, so a
   * caller written before this field keeps working unchanged. What this
   * adds is the difference between the two failures that used to look
   * identical: a key that is not in the remote authorized_keys, and an
   * account that authenticates and cannot read the folder. Those are
   * different problems on different machines, and collapsing them is how
   * somebody ends up regenerating a keypair to fix a chmod.
   *
   * Empty on a deployment that predates the field, which a render site
   * has to treat as "no breakdown reported" rather than as four failures.
   */
  stages: ConnectionTestStage[];
}

/** One of the four claims. `status` is "passed", "failed" or "skipped",
 *  and "skipped" is a result rather than an absence: a stage never
 *  attempted because an earlier one failed is a different thing from a
 *  stage that failed, and rendering them the same way would show three
 *  reds for one problem. */
export interface ConnectionTestStage {
  name: "connect" | "host_key" | "authenticate" | "list";
  status: "passed" | "failed" | "skipped";
  /** A sanitized, safe-to-render sentence from the server. */
  detail?: string;
  durationMs: number;
}
