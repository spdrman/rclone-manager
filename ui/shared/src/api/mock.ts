...
/** A verification where all four claims hold, in the order they are
 *  proven. Four rather than a bare `ok`, because a fixture that answered
 *  with only the boolean would let a page that ignores the breakdown look
 *  correct in every browser test. */
function mockPassingStages(user: string): ConnectionTestStage[] {
  return [
    { name: "connect", status: "passed", detail: "tcp prod-db-01.internal:22 · SSH-2.0-OpenSSH_9.6p1", durationMs: 210 },
    { name: "host_key", status: "passed", detail: "ssh-ed25519 " + mockProbedFingerprint, durationMs: 18 },
    { name: "authenticate", status: "passed", detail: "publickey as " + user, durationMs: 96 },
    { name: "list", status: "passed", durationMs: 41 }
  ];
}
...
export function createMockApi(scenario: Scenario = "default"): BackupManagerApi {
  ...
  const api: BackupManagerApi = {
    ...
    testConnection: () => delay({ ok: true, stages: mockPassingStages("backup-agent") }),
    ...
    testCandidateConnection: (params): Promise<ConnectionTestOutcome> =>
      delay({ ok: true, stages: mockPassingStages(params.user) }),
