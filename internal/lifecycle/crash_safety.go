...
// hold, not about this package's own code — this package cannot enforce
// I/O ordering it doesn't perform, only the state graph around it.
//
...
// So "during" a journal write never produces a torn or partial value —
// a restart observes either the old state (the write never landed) or the
// new one (it did). That collapses every "before / during / after"
// question into two real cases, except where a transition also straddles
// a call to something outside the journal (the local filesystem, or the
// remote). Those cases are called out explicitly below.
//
// # COMMITTING -> COMMITTED
//
// This is FR-11's "durably commit local file" step: fsync the local file,
// rename it from its .partial name to its final name, fsync the
// directory, then record COMMITTED.
//
...
//     has told any other component the artifact is committed, so a
//     restart re-runs the whole durable-commit step from scratch. Both
//     fsync and a rename to the same final name are safe to repeat.
...
//     the fact. The remote is still completely untouched. This is the
//     safe state the whole design exists to reach before anything
//     irreversible can happen to the remote copy.
//
...
//     still exists. This is a durable statement of intent, recorded
//     before anything destructive happens, so a crash right here just
//     means the delete hasn't been tried yet.
//
...
//
//   - Before the remote delete call: the journal reads
//     REMOTE_DELETE_PENDING, the remote object still exists, untouched.
//     Safe: nothing has happened yet.
//   - During the remote delete call: unknown from the caller's side
//     whether it took effect. This is fine because FR-16 requires
//     re-comparing the remote object's identity against what was recorded
//     at discovery immediately before (re-)issuing the delete. A restart
//     that finds the journal still at REMOTE_DELETE_PENDING re-runs that
//     check:
//     -- same identity still present: issue the delete (again, if this is
//     a retry — deleting a positively re-identified object is safe to
//     repeat);
...
//     and delete blind.
//   - During the journal write to COMPLETE: atomic, and only ever
//     attempted after the remote side of the story above is already
//     resolved one way or another. A restart observes REMOTE_DELETE_PENDING
//     (re-run the check-then-maybe-delete step above, which is idempotent)
//     or COMPLETE (done).
...
//     recorded before any delete call — the ordering FR-10 exists to
//     guarantee, and the one TestOnlyCommittedPrecedesRemoteDeletePending
//     proves at the graph level.
//
// # Reading this against the two failure shapes that would be a design bug
//
// "The artifact is stuck": can't happen, because every state this package
// defines has at least one declared outgoing edge (TestNoStateIsALeak
// checks all eleven), including FAILED and QUARANTINED.
//
// "The remote may be deleted without a committed local copy": can't
// happen, because REMOTE_DELETE_PENDING — the only state a delete call may
// ever be issued from — has exactly one legal predecessor, COMMITTED
